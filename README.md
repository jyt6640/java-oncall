...
## 🔍 진행 방식
<img width="978" height="467" alt="제목 없음" src="https://github.com/user-attachments/assets/977782ed-c2e9-46dc-9a65-81ad14b2ae17" />
...
- [X] 월 ~ 일 요일 생성
- [X] 다음 날짜를 반환하는 기능
- [X] 문자열로 입력하여 Day를 반환하는 기능

#### WorkDays 도메인 (일급 컬렉션)

- [X] 월, 시작 요일로 객체 생성
- [X] List<WorkDay> 리스트 생성
- [X] WorkDay 도메인을 생성하는 기능
- [X] 달에 몇일이 있는지 반환하는 기능

#### 근무 배정 서비스

- [X] 근무 배정 기능
- [ ] 휴일 - 평일 연속으로 근무 시 평일 순서를 바꾸어서 추가하는 기능
- [ ] 평일 - 휴일 연속으로 근무 시 휴일 순서를 바꾸어서 추가하는 기능
- [X] Map으로 요일과 근무자 매칭 기능
...
- [X] 월 입력 값이 '1~12' 사이가 아닐 경우 예외 발생
- [X] 요일 입력 값이 '월~일'이 아닐 경우 예외 발생
- [ ] 동일한 사원의 이름을 각각의 입력에서 중복될 경우 예외 발생
- [ ] 입력 값이 공백일 경우 예외 발생
- [ ] 근무 순번을 쉼표를 기준으로 입력하지 않을 경우 예외 발생
